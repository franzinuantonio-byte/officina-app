...
NOME_FILE_CHIAVE = "chiave.json"

# NOMI FOGLI (Devono esistere su Google!)
...
def get_google_sheet():
    try:
        gc = gspread.service_account(filename=NOME_FILE_CHIAVE)
...
def get_worksheet_sicuro(sh, nome_foglio, colonne_default):
    try:
        ...
        # Se non esiste, prova a crearlo (ma meglio se esiste già)
...
def carica_dati(nome_worksheet, colonne_attese):
    try:
        ...
        # Converte tutto in stringa per evitare problemi
        df = df.astype(str)
        ...
        st.error(f"Errore lettura {nome_worksheet}: {e}")
...
def salva_dati(df, nome_worksheet, colonne_attese):
    try:
        ...
        worksheet = get_worksheet_sicuro(sh, nome_worksheet, colonne_attese)
        worksheet.clear()
        # Scrive le colonne + i dati
        dati = [df.columns.values.tolist()] + df.astype(str).values.tolist()
        worksheet.update(dati)
    except Exception as e:
        st.error(f"❌ Errore salvataggio su Google: {e}")
...
def add_bg():
    ...
    <style>
    .stApp {
        background-image: url("https://images.unsplash.com/photo-1530046339160-ce3e4234721f?q=80&w=1920&auto=format&fit=crop");
    ...
    with st.container():
        ...
        if st.button("INIZIA 🚀", type="primary", use_container_width=True):
            if ta and de:
                ...
                if not df_in_corso[df_in_corso['Operatore'] == op].empty:
                    st.error("Operatore occupato!")
                else:
                    nuovo = {
                        "Targa": ta, 
                        "Operatore": op, 
                        "Lavori da Eseguire": de, 
                        "Ora Ultimo Inizio": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 
                        "Minuti Gia Fatti": "0", 
                        "Stato": "IN CORSO"
                    }
            ...
            with st.container():
                ...
                if r['Stato'] == "IN CORSO":
                    ...
                    if c4.button("FINITO 🏁", key=f"f{i}", type="primary"):
                        fine = datetime.now()
                        storic = {
                            "Data": fine.strftime("%Y-%m-%d"), 
                            "Targa": r['Targa'], 
                            "Operatore": r['Operatore'], 
                            "Lavori Eseguiti": r['Lavori da Eseguire'], 
                            "Ora Inizio": "--", 
                            "Ora Fine": fine.strftime("%H:%M"), 
                            "Durata (min)": str(m_tot)
                        }
...
# --- AGENDA ---
    ...
    with st.form("ag"):
        ...
        if st.form_submit_button("Salva Appuntamento"):
            if t and l:
                ...
                st.success("Appuntamento salvato su Google!")
                time.sleep(1)
                st.rerun()
            else:
                st.warning("Inserisci almeno Targa e Lavoro!")
    ...
    if not df_agenda.empty: 
        ...
        st.info("Agenda vuota su Google.")
