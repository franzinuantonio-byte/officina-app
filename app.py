...
if menu == "⏱️ Officina":
    ...
    col_date, col_filler = st.columns([1, 2])
    data_lavoro = col_date.date_input("📅 Data Appuntamenti", value=date.today())
    data_str = str(data_lavoro)
    
    app_del_giorno = pd.DataFrame()
    if not df_agenda.empty and 'Data' in df_agenda.columns:
        app_del_giorno = df_agenda[df_agenda['Data'] == data_str]
    
    opts = ["-- Manuale --"]
    if not app_del_giorno.empty:
        opts += [f"{r['Ora']} | {r['Targa']} | {r['Lavoro']}" for i, r in app_del_giorno.iterrows()]
    ...
    if sel != "-- Manuale --":
        try:
            parti = sel.split(" | ")
            t_sel = parti[1]
            # Ricerca sicura della riga
            row = app_del_giorno[app_del_giorno['Targa'] == t_sel].iloc[0]
            t_def = row['Targa']
            l_def = f"{row['Lavoro']}"
            if row['Scaffale Ricambi']:
                l_def += f" [Scaffale: {row['Scaffale Ricambi']}]"
        except: pass
...
# --- MENU 3: ADMIN ---
    ...
    if pwd == "admin":
        ...
        if not df_storico.empty:
            ...
            st.info("Nessun lavoro nello storico.")
